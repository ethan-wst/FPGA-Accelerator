# resnet_benchmark.py
# Benchmark pretrained ResNet models on random images from ImageNet_SubSet to measure inference speed and top-1/top-5 accuracy
...
# --- Argument Parser ---
parser = argparse.ArgumentParser(description='Benchmark ResNet50 inference speed and accuracy')
parser.add_argument('--model', type=str, default='resnet50', choices=['resnet50', 'resnet50_quant'], help='Model name: resnet50 or resnet50_quant')
...
# Calculate model size - accounting for quantization (8-bit ints vs 32-bit floats)
...
total_size_bytes = quant_params + float_params * 4  # 1 byte per int8, 4 bytes per float32
